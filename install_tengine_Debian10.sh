...
apt-get install libpcre3 libpcre3-dev wget libssl-dev gcc make libxslt-dev libgd-dev libxml2-dev libgeoip1 libjemalloc-dev git tar
#yum -y install pcre-devel wget openssl-devel gcc make libxslt-devel libxml2-devel gd-devel GeoIP-devel jemalloc-devel wget gcc make git tar
cd $base
...
user nobody nogroup; # 出于安全，无特别要求禁止使用root
